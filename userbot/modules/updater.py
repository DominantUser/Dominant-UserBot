...
async def upstream(ups):
    ...
    await ups.edit("`Güncellemeler denetleniyor...DominantUserBot`")
    ...
    try:
        ...
        repo.heads.dominant.set_tracking_branch(origin.refs.sql)
        repo.heads.dominant.checkout(True)

    ac_br = repo.active_branch.name
    if ac_br != 'master':
        await ups.edit(
            f'**[Güncelleyici]:**` Galiba Dominant botunu modifiye ettin ve kendi branşını kullanıyorsun: ({ac_br}). '
            'Bu durum güncelleyicinin kafasını karıştırıyor,'
            'Güncelleme nereden çekilecek?'
            'Lütfen Dominant botunun resmi repodan kullan.`')
        repo.__del__()
        return
    ...
    if not changelog and not force_update:
        await ups.edit(
            f'\n`Botun` **tamamen güncel!** `Dominant:` **{ac_br}**\n')
        repo.__del__()
        return

    if conf != "now" and not force_update:
        changelog_str = f'**{ac_br} UserBot için yeni güncelleme mevcut!\n\nDeğişiklikler:**\n`{changelog}`'
        ...
        await ups.respond('`Güncellemeyi yapmak için \".update now\" komutunu kullan.\"Güncelleme yaklaşık 10 15 dakika arası sürer."\"CeteUserBot.`')
        return
    ...
    if HEROKU_APIKEY is not None:
        ...
        await ups.edit('`[HEROKU MEMEZ]\
                        \nDominantUserBot Heroku dynosuna aktarılıyor, lütfen bekle...`'
                       )
        ups_rem.fetch(ac_br)
        ...
        await ups.edit('`Güncelleme başarıyla tamamlandı!DominantUserBot\n'
                       'DominantUserBot Yeniden başlıyor...`')
