...
async def promote(promt):
    ...
    if not rank:
        rank = "Administración"  # Her ihtimale karşı.
    if user:
        pass
    else:
        return
...
async def ban(bon):
    ...
    await bon.edit("`Düşman Dominant Tarafından vuruldu!`")
...
async def nothanos(unbon):
    ...
    await unbon.edit("`DominantBot Yasağı Kaldırıyor Lütfen Bekleyiniz...`")
...
async def spider(spdr):
    ...
    await spdr.edit("`DominantBot Kullanıcıyı Susturuken Lütfen Bekleyin...`")
...
async def unmoot(unmot):
    ...
    await unmot.edit('```DominantBot Kullanıcıyı Sessizden çıkartıyor...```')
...
async def ungmoot(un_gmute):
    ...
    await un_gmute.edit('```DominantBot Tarafından Küresel susturma kaldırılıyor...```')
...
async def gspider(gspdr):
    ...
    # Başarı olursa bilgi ver
    await gspdr.edit("` Kullacıyı Küresel Susturuyor...`")
...
async def rm_deletedacc(show):
    ...
    if con != "clean":
        await show.edit("`DominantBot hayalet / silinmiş / zombi hesapları aranıyor...`")
    ...
    await show.edit("`DominantBot Silinmiş hesapları çıkartıyor...`")
    del_u = 0
    del_a = 0
...
async def pin(msg):
    ...
    await msg.edit("`DominantBot Mesajı Başarıyla sabitledi!`")
...
async def kick(usr):
    ...
    await usr.edit("`DominantBot Trafından Kullanıcı Çıkartılıyor...`")
